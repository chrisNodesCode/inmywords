...
- **Budget (`/chris/budget`)** — a forecasting budget ported (conceptually) from Airtable's `categories`/`items`/`calendar`. Models: `BudgetCategory` (typed `BudgetKind` = `DEBIT`/`CREDIT`, optional `color`), `BudgetItem` (`name` + `amount` Float USD + optional `categoryId`, `onDelete: SetNull`; the amount is the running-total anchor), `BudgetEntry` (the "calendar": one planned occurrence with a `date`, FK `itemId` `onDelete: Cascade`). **Sign convention:** `CREDIT` adds, `DEBIT`/uncategorized subtracts. Month is **derived from `entry.date`** (no month table). Routes: owner-gated CRUD under `/chris/api/budget/{categories,items,entries}` (+ `[id]`); entries GET supports `?from=&to=`. v1 UI is a read-only month-grouped inspection table (subtotals + running balance). Airtable data port is pending (the connected MCP token can't see base `appXbSIYO4c4wBlyN`).
