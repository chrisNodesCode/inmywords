...
- **Modules:** To-dos, Calendar, Projects, Journal (shared with IMW), Shopping, Prompts, **Messaging** (`/chris/messages`), and **Budget** (`/chris/budget`). Module cards are listed in `app/chris/page.tsx`.
- **Budget (`/chris/budget`)** — a forecasting budget ported from Airtable's `categories`/`items`/`calendar`. Models: `BudgetCategory` (typed `BudgetKind` = `DEBIT`/`CREDIT`, optional `color`), `BudgetItem` (`name` + `amount` Float USD + optional `categoryId`, `onDelete: SetNull`; the amount is the canonical running-total anchor), `BudgetEntry` (the "calendar": one planned occurrence with a `date`, FK `itemId` `onDelete: Cascade`, **`amountOverride Float?`**). **Effective amount** = `entry.amountOverride ?? item.amount`; **sign** from category `kind` (`CREDIT` adds, `DEBIT`/uncategorized subtracts). The override delinks just one occurrence's value — editing an item's canonical `amount` updates every instance except overridden ones (set inline in the row; `↺` relinks). Month is **derived from `entry.date`** (no month table). Routes: owner-gated CRUD under `/chris/api/budget/{categories,items,entries}` (+ `[id]`); entries GET supports `?from=&to=`, entries PATCH/POST accept `amountOverride` (number sets, `null` relinks). UI: month-grouped table (subtotals + running balance), row click → entry modal, **Manage** modal (items + categories CRUD), inline-editable amount cell. Data: ported via `scripts/port-budget.ts` (real data lives under the owner's Clerk id for prod **and** `dev-user-local` for the local preview — two independent copies). **Accounts:** `BudgetAccount` (unique `userId`+`key` = `checking`/`savings`, editable `balance`) holds the manually-tracked actual balances (no bank API). `GET /chris/api/budget/accounts` get-or-seeds both; `PATCH` updates a balance by key. The two legacy `CHECKING CURRENT`/`SAVINGS CURRENT` line items were converted into accounts (and removed from the calendar) via `scripts/convert-budget-accounts.ts` (run per userId; the port script now routes those rows straight to accounts). **Views (tabbed, extensible):** *Calendar* = all entries, running balance starts at the Checking balance (this makes our running totals match the source Airtable); *Savings* = entries in the `SAVINGS` category only, running balance starts at the Savings balance with the **sign flipped** (a checking-debit transfer = a savings deposit). Editing an account card recomputes the active view's running totals live.
